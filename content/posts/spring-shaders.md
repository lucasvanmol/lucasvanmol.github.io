...
In a small game called Hook Head I made for a game jam, the main character has a spring-like appendage which reacts organically to the character's movement. This post will talk about this effect and also how to create the same effect in 3D. A demo of the contents of this post is available on GitHub.


Hook HeadEiffel Tower model by cbmbeach - licensed under CC BY-SA 3.0
...
Such a matrix can be used in a shader by multiplying it by the UV's in order to rotate the texture by an angle θ. For a bending effect, we can let the angle of rotation at each pixel be proportional to the euclidean distance between the pixel and some center point. This means that the further away a pixel is from the center point, the more the pixel gets rotated and curved away from its original position - also known as a twirl. We can multiply the distance by some strength value to control how much twirl we want.
...
Modified twirl shader and added padding
...
Combining the shader and the oscillator script
...
Playing around with the values of spring and damping constants will result in different effects. The spring constant controls how much the oscillator will be pulled back towards the equilibrium position - essentially how 'springy' it is.

Changes in spring constant, high to low - notice how they all come to a stop at the same time
...
The normals are easily recalculated by appling the same rotation without the offsetHere we can see how each matrix affects the mesh:

Eiffel Tower model by cbmbeach - licensed under CC BY-SA 3.0
...
This post has been much longer than I anticipated, but I'll quickly go over some optimizations that can be made. The first is precalculating the multiplication of the rotation matrices. But while transformations using matrices may be conceptually easier to understand, rotations can also be done with quaternions, which are more efficient if all you want to do is rotate. How quaternions work is quite out of scope of this post, but here's an example of how you could implement them.

For another approach to boingy buildings, check out this great post by GlassTome Games, which uses Bezier curves for rotation.
